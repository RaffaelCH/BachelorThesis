...
// Color depending on language.
const LanguageBasedColor = Object.freeze({
	'Java': '#006000',
	'Python': '#00FF00'
});
...
const ColorHelper = {
    ...
    getColorByStatus: node => {
        if (NodeType.isReferencedType(node.type)) {
            ...
            return '#C0C0C0';
        } else if (node.type == NodeType.UNKNOWNFILE) {
            return '#80D0FF';
        }
	...
	getColorByLanguage: language => {
		return LanguageBasedColor[language]
	},
    ...
    getColor: (node, isChangeBasedMode) => {
        return isChangeBasedMode ? ColorHelper.getColorByPackage(node.packageName) : ColorHelper.getColorByStatus(node);
    },
